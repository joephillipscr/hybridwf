import HwfsTool from '../HwfsTool';
import Reveal from '../Reveal';
import { PageHeader, Section, SectionHeading } from '../Section';
import { AUTONOMY, AUTONOMY_NOTE } from '@/lib/hwfs';
import type { Locale } from '@/lib/site';

const C = {
  eyebrow: { en: 'Instrument', es: 'Instrumento' },
  title: { en: 'Hybrid Workforce Fit Score', es: 'Hybrid Workforce Fit Score' },
  lede: {
    en: 'Decide whether a responsibility should be Human, Artificial or Hybrid — and at what autonomy it should start. Twelve dimensions, one responsibility at a time.',
    es: 'Decidí si una responsabilidad debe ser Humana, Artificial o Híbrida — y en qué autonomía debería empezar. Doce dimensiones, una responsabilidad por vez.',
  },
  whyTitle: { en: 'Why there is no score', es: 'Por qué no hay puntaje' },
  why: {
    en: [
      'A number would be easier to build and easier to abuse. A manager who has already decided can pick weights until the arithmetic agrees, and then present the result as objectivity. The instrument would become the justification rather than the analysis.',
    ...
    es: [
      'Un número sería más fácil de construir y más fácil de abusar. Un manager que ya decidió puede elegir pesos hasta que la aritmética le dé la razón, y después presentar el resultado como objetividad. El instrumento se convertiría en la justificación en vez del análisis.',
...
export default function HwfsPage({ lang }: { lang: Locale }) {
  return (
    <>
