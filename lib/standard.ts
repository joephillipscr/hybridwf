...
export const CLAUSES: Clause[] = [
  ...
  },
  {
    ...
    note: {
      en: 'The post exists before its occupant. A name, a tone of voice and a set of instructions describe a persona; a role states what result must be produced, with what authority, measured how. Before does not mean frozen: a post may be reshaped by whoever holds it — Taylor fixed the person to the box, this standard versions the box — and HWF-23 exists so that the reshaping happens as declared revision rather than tacit drift.',
  ...
  },
  {
    ...
    note: {
      en: 'Proportionality is the rule: the higher the cost of the error and the harder it is to reverse, the more approval the action carries. Separation of duties applies, so whoever initiates need not approve. What counts as high-risk is not left to taste: it resolves against the risk classification of HWF-21.',
      es: 'La regla es proporcionalidad: a mayor costo del error y menor reversibilidad, más aprobación soporta la acción. Aplica separación de funciones, así que quien inicia no necesariamente aprueba. Qué cuenta como alto riesgo no queda al gusto: se resuelve contra la clasificación de riesgo de HWF-21.',
    },
  },
  {
    id: 'HWF-15',
...
/** The nine-field conformance declaration (HWF-27). */
...
/** The five-tier risk classification (HWF-21). */
...
export const RISK_CLASSES: RiskTier[] = [
  {
    ...
    meaning: {
      en: 'A use that cannot be made conformant by any control: it requires deceiving people about the system (HWF-08), overriding the constraints of HWF-01, or operating outside any accountable chain.',
      es: 'Un uso que no puede volverse conforme mediante ningún control: exige engañar a las personas sobre el sistema (HWF-08), pasar sobre las restricciones de HWF-01, u operar fuera de toda cadena accountable.',
    },
    regime: {
...
export const RISK_NOTE: T = {
  ...
  es: 'El HWFA deriva una clase provisional de costo del error y reversibilidad al asignar; la clase declarada de HWF-21 pesa los siete factores y prevalece. Los niveles están diseñados para interoperar con regímenes basados en riesgo como el EU AI Act sin afirmar equivalencia jurídica: mapear una clase a una categoría legal es un ejercicio de abogados, no una propiedad de este documento.',
};
...
export const BLOCKS: { numeral: string; title: T; from: number; to: number }[] = [
  { numeral: 'I', title: { en: 'Human foundation', es: 'Fundamento humano' }, from: 1, to: 4 },
  { numeral: 'II', title: { en: 'The category', es: 'La categoría' }, from: 5, to: 8 },
  { numeral: 'III', title: { en: 'Accountability', es: 'Accountability' }, from: 9, to: 11 },
  { numeral: 'IV', title: { en: 'Authority and operational control', es: 'Autoridad y control operativo' }, from: 12, to: 16 },
  { numeral: 'V', title: { en: 'Evidence and data', es: 'Evidencia y datos' }, from: 17, to: 20 },
  { numeral: 'VI', title: { en: 'Risk and validation', es: 'Riesgo y validación' }, from: 21, to: 22 },
  { numeral: 'VII', title: { en: 'Lifecycle', es: 'Ciclo de vida' }, from: 23, to: 26 },
  { numeral: 'VIII', title: { en: 'Conformance', es: 'Conformidad' }, from: 27, to: 27 },
];
...
export const CONFORMANCE: { title: T; body: T[] } = {
  ...
  body: [
    ...
    {
      en: 'The clause text is the citable unit; cite it by identifier, so HWF-20 rather than a page number. Notes are commentary and may change between versions without amending the standard.',
      es: 'El texto de la cláusula es la unidad citable; citala por identificador, o sea HWF-20 y no un número de página. Las notas son comentario y pueden cambiar entre versiones sin enmendar el estándar.',
    },
  ],
};
