...
export const MODEL: { title: T; rule: T; body: T } = {
  ...
  rule: {
    en: 'The Editor writes, proposes and decides everything editorial. From the freeze, the normative text changes only by vote of the Board — and the Editor has a voice, not a vote. Until the freeze, the Candidate is a working draft under the Editor’s custody, and its drafting history is public, commit by commit, in the repository.',
    es: 'El Editor escribe, propone y decide todo lo editorial. Desde el congelamiento, el texto normativo cambia solo por votación del Board — y el Editor tiene voz, mas no voto. Hasta el congelamiento, la Candidate es un borrador bajo custodia del Editor, y su historial de redacción es público, commit por commit, en el repositorio.',
  },
  body: {
    en: 'That single rule is what makes the disclosure above worth anything: the author holds the largest commercial interest in the room. Until the freeze he holds editorial custody, and says so; from the freeze he cannot move the standard — only persuade the people who will be able to, once the Board exists.',
    es: 'Esa única regla es la que le da valor a la divulgación anterior: el autor tiene el mayor interés comercial de la sala. Hasta el congelamiento conserva la custodia editorial, y lo dice; desde el congelamiento no puede mover el estándar — solo persuadir a quienes sí podrán, cuando el Board exista.',
  },
};
...
export const RELEASES: Release[] = [
  {
    ...
    date: '2026-08-12',
    status: { en: 'Working draft under editorial custody until the freeze on 1 September 2026 — pending Board ratification', es: 'Borrador en curso bajo custodia editorial hasta el congelamiento del 1 de septiembre de 2026 — pendiente de ratificación del Board' },
    notes: [
      ...
      { en: 'Certification is deliberately outside the scope and does not return: the author holds a commercial interest in the category, so the standard scores no products, issues no seals, and makes conformance a self-declaration anyone can verify or refute — public scrutiny, structure instead of trust.', es: 'La certificación está deliberadamente fuera del alcance y no regresa: el autor tiene un interés comercial en la categoría, así que el estándar no puntúa productos, no emite sellos, y convierte la conformidad en una autodeclaración que cualquiera puede verificar o refutar — escrutinio público, estructura en vez de confianza.' },
