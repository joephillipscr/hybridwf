...
El texto de la cláusula es la unidad citable; citala por identificador, o sea HWF-20 y no un número de página. Las notas son comentario y pueden cambiar entre versiones sin enmendar el estándar.

### La declaración de conformidad

Toda declaración de conformidad publica estos nueve campos (HWF-27). Una declaración a la que le falte cualquiera no es una declaración de conformidad bajo este estándar. El campo de clase de riesgo carga su razonamiento factor por factor, no solo la etiqueta. Y una declaración publicada es una representación comercial, accionable bajo el derecho del consumidor y de competencia: sustanciala antes de publicar, y retirala o corregila pronto cuando venza o falle.
...
*Nota:* HWF-21 clasifica por juicio, y el juicio puede estar motivado: sin un piso, una organización bajo presión de costos clasifica las decisiones de despido como Moderadas y deja que la cola decida. Esta lista es el piso que ninguna evaluación puede bajar — los asuntos donde equivocarse cae sobre una persona y no sobre un libro contable. La segunda mitad de la cláusula existe porque el human-in-the-loop degenera por defecto: un aprobador frente a doscientas recomendaciones al día, cada una pre-puntuada y pre-redactada, aprueba a un ritmo que vuelve imposible entender, y el sesgo de automatización hace el resto. Eso no es supervisión; es la ceremonia de la supervisión. La prueba de una decisión real es operativa: el humano puede reconstruir los determinantes — HWF-17 existe para entregarle el material — tiene la autoridad y el tiempo para decidir distinto, y una decisión divergente no carga penalidad por defecto. Donde el ritmo de aprobación vuelve imposible reformular el caso, la organización automatizó la decisión y conservó una firma humana: responsabilidad perdida y no delegada, en términos de HWF-09. La lista interopera con el Artículo 22 del GDPR y los requisitos de supervisión humana del EU AI Act en la misma postura que la clasificación de riesgo — diseñada para viajar, sin afirmar equivalencia jurídica.
...
**La accountable ownership es una capacidad dotada de recursos, no un nombre en un campo. El accountable owner debe tener competencia sobre el dominio, autoridad efectiva, tiempo y capacidad para el alcance que supervisa, acceso directo a la evidencia, independencia suficiente de las presiones que son dueñas del resultado, el poder de suspender el recurso, y entrenamiento sobre sus limitaciones y sobre el sesgo de automatización. El mecanismo de suspensión debe ejercitarse con una cadencia declarada — nunca mayor a doce meses, y más corta para puestos Altos y Críticos — porque un kill switch que nunca se ha jalado es decoración. La accountability no debe descargarse sobre el humano que supervisa: donde los determinantes de una falla llevan a la política, el diseño, el tooling o el deployment, la responsabilidad aterriza ahí, y un humano colocado al final de un proceso no es una zona de absorción para culpa que pertenece aguas arriba.**

*Nota:* La nota de HWF-10 ya había nombrado la falla: mil AI Employees sin un instrumento capaz de mostrar qué hizo cada uno son un organigrama y no accountability. Esta cláusula declara las condiciones que vuelven real la ownership, y dos merecen atención. La independencia es la filosa: un dueño cuyo bono depende del throughput que el recurso produce no lo va a suspender jamás — la supervisión y la presión de resultado deben separarse, que es la separación de funciones de HWF-14 elevada al nivel del rol. Tiempo y capacidad es la silenciosa: el span of control es un principio adaptado, así que la escala solo crece contra tooling, y un dueño de cuarenta recursos sin dashboard es dueño de una lista. El ejercicio de suspensión es la disciplina del simulacro de incendio: ejercitado con cadencia declarada y registrado, donde una falla descubierta en ejercicio es un hallazgo y una descubierta en producción es un incidente — la primera jalada real no puede ser la primera jalada. La prohibición de zona de absorción toma su nombre del estudio de Elish sobre cómo la culpa en sistemas automatizados aterriza en el humano más cercano mientras el control estaba aguas arriba, y su mecanismo de HWF-17: la culpa sigue a los determinantes, no a la proximidad. El humano que supervisa responde por lo que controló — su revisión, su intervención — y lo que no podía controlar aterriza en quien lo poseía. Esto además completa HWF-09 desde el otro lado: portar una consecuencia exige haber tenido control, y consecuencia sin control no es accountability sino chivo expiatorio — un rol que ninguna persona competente aceptaría, que es exactamente por lo que la prohibición protege al dueño tanto como protege la verdad.

### IV · Autoridad y control operativo
...
*Nota:* La regla es proporcionalidad: a mayor costo del error y menor reversibilidad, más aprobación soporta la acción. Aplica separación de funciones, así que quien inicia no necesariamente aprueba. Qué cuenta como alto riesgo no queda al gusto: se resuelve contra la clasificación de riesgo de HWF-21.
...
*Nota:* La batería de pruebas presupone un sistema estocástico — algo que razona, y que por tanto puede driftear, alucinar y ser inyectado. La automatización determinista no hace nada de eso, y nada de esta cláusula la tocó jamás: la automatización está peldaños debajo de AI Employee en la escalera de vocabulario y falla el test de nueve propiedades, así que las obligaciones se atan a la categoría y la categoría excluye el determinismo. Exigirle monitoreo de drift a un flujo RPA es exigirle un test de embarazo a una piedra. Entre AI Employees el dial es la clase de riesgo, no el todo-o-nada: un deployment de riesgo Bajo revalida casos, excepciones y regresión, y la batería adversarial y de sesgo llega de Alto hacia arriba, porque HWF-21 existe precisamente para que las obligaciones escalen. La cláusula cierra una asimetría que el estándar cargaba: el nacimiento está exquisitamente compuertado — doce pasos, shadow mode, performance gate, una transición ganada con evidencia — mientras el cambio no tenía compuerta alguna, y un swap de modelo podía llegar a producción sin probarse. Los doce pasos ganan la transición; esta cláusula la mantiene ganada. Los riesgos de equipo existen porque HWF-10 permite pirámides: la delegación puede circular, el feedback puede componerse, la memoria puede contaminar el contexto aguas abajo, la autoridad puede propagarse lateralmente por los handoffs — y el riesgo menos intuitivo es la falla correlacionada, porque cien humanos se equivocan de cien maneras distintas mientras cien instancias de un mismo modelo se equivocan idénticamente y a la vez. La monocultura de modelo es el único riesgo que la administración clásica nunca tuvo; la diversidad y el fallback son su mitigación. Sobre el método, este estándar dice qué debe validarse y cuándo; ISO/IEC 42001 y 42005 y el NIST AI RMF aportan sistemas de gestión para el cómo, y este documento los complementa en vez de recrearlos — la misma subsidiariedad que aplica al derecho laboral.
...
| --- | --- | --- |
| Prohibido | Un uso que no puede volverse conforme mediante ningún control: exige engañar a las personas sobre el sistema (HWF-08), pasar sobre las restricciones de HWF-01, u operar fuera de toda cadena accountable. | No se realiza bajo este estándar, con ninguna configuración de recursos. |
...
El HWFA deriva una clase provisional de costo del error y reversibilidad al asignar; la clase declarada de HWF-21 pesa los siete factores y prevalece. Los niveles están diseñados para interoperar con regímenes basados en riesgo como el EU AI Act sin afirmar equivalencia jurídica: mapear una clase a una categoría legal es un ejercicio de abogados, no una propiedad de este documento.
...
| 94 | Protección de datos | Principios de privacidad, acceso, minimización y retención. | Data governance, consent, retrieval filters, retention y deletion — cubriendo las inferencias derivadas y los propios logs de auditoría (HWF-18). | U |
...
| 101 | Criterios de terminación | Bajo desempeño, reestructuración, incumplimiento u otras causas. | Obsolescencia, costo, riesgo, incidents, bajo desempeño o cambio de arquitectura; la existencia se re-justifica con cadencia declarada (HWF-25). | U |
...
6. Garantizar que todo AI Employee tenga Role Contract, manager, KPIs, permisos, límites, audit trail, performance review, y un kill switch ejercitado con cadencia declarada (HWF-11).
...
**Los acrónimos no se traducen. WRM, HWFA y los identificadores de cláusula HWF- se mantienen idénticos en toda edición de este estándar, presente y futura; solo se localizan las palabras que expanden. Quien cite WRM o HWF-20 en cualquier idioma está señalando lo mismo.**
...
**G-06 · Context Provisioning** — El equivalente al onboarding para un recurso artificial. Se divide en debe saber, puede consultar y no debe acceder. La tercera categoría tiene dos fundamentos legítimos: proteger la información del recurso, y proteger la decisión de la información (HWF-16); en ambos casos la restricción queda registrada, nunca silenciosa.
...
**G-41 · Falla correlacionada** — El modo de falla de la monocultura de modelo: cien humanos se equivocan de cien maneras distintas, mientras cien AI Employees corriendo el mismo modelo o stack de proveedor se equivocan idénticamente y a la vez, compartiendo puntos ciegos, sesgos y exploits. El único riesgo que la administración clásica nunca tuvo. Se mitiga con diversidad de modelos y proveedores, recursos de fallback y planes de sucesión; nombrado como ítem de validación para equipos de IA por HWF-22.
...
| 8 | Accesibilidad e inclusión | La restricción de accesibilidad de HWF-01 y el factor de personas vulnerables de HWF-21 y HWF-02. Un practicante de sistemas accesibles, no un auditor de documentos. | *Abierto* |
...
8. Cómputo solo agregado de las métricas del scorecard que tocan humanos con nombre, con el acceso a nivel individual compuertado bajo la prueba de propósito nuevo de HWF-18.
...
- El Fit Score se convirtió en el Fit Assessment — HWFA — y se reconstruyó en tres etapas. Primero el renombre: un instrumento que deliberadamente se niega a producir un número no debería llamarse Score, y retirar el acrónimo viejo disuelve además cualquier confusión residual entre el instrumento y el estándar. El rediseño corrige tres defectos que una revisión atrapó contra el código en ejecución: la dimensión de economía era circular — preguntaba qué alternativa gana y devolvía la respuesta como análisis — y ahora es un perfil de costos factual consultado solo en la etapa final; la enumerabilidad completa puntuaba hacia la IA probabilística cuando es la firma del territorio del software determinista, y el instrumento ahora tiene un cuarto resultado — automatización determinista — para trabajo por reglas con cola de excepciones delgada, porque un estándar de AI Employees cuyo instrumento sabe decir «no necesitás uno» es más difícil de descartar como hype; y el volumen jalaba hacia la automatización cuando la escala multiplica el daño, así que ahora amplifica la clase de riesgo. Las etapas encarnan el orden de HWF-20 — elegibilidad, luego riesgo, luego economía — abriendo con una dimensión nueva de materias reservadas cableada a HWF-23, y la dimensión de empatía ampliada a significancia humana: dignidad, vulnerabilidad y poder sobre la persona, no solo valor de la relación. Los cinco resultados planos de la propuesta no se adoptaron: los grados asistidos son lo que la asignación cruzada con la escalera de autonomía ya expresa. Trece dimensiones; sigue sin haber número.
- HWF-27 compuerta el cambio como el playbook compuerta el nacimiento: una versión es un registro, no una prueba, y todo cambio material — modelo, herramientas, políticas, conocimiento, autoridad — se revalida antes de operar, a profundidad fijada por la clase de riesgo. La objeción del autor a la propuesta modeló la cláusula: la batería de pruebas presupone un sistema estocástico, y la automatización determinista está debajo de la categoría AI Employee en la escalera de vocabulario, así que las obligaciones nunca la tocan — mientras entre AI Employees el dial es la clase de HWF-22 y no el todo-o-nada, con la batería adversarial y de sesgo llegando de Alto hacia arriba. Los riesgos sistémicos de equipo quedan nombrados, la falla correlacionada ante todo: la monocultura de modelo es el único riesgo que la administración clásica nunca tuvo. ISO/IEC 42001 y 42005 y el NIST AI RMF se complementan, no se recrean — la misma subsidiariedad que el estándar aplica al derecho laboral. Agrega G-41 y ambos estándares ISO a las fuentes.
- HWF-08 se extendió de la memoria a toda la superficie de datos del deployment: inputs, outputs, resultados de herramientas, memoria e inferencias derivadas, cada uno con finalidad y base legal declaradas, minimización, controles de calidad, manejo de datos sensibles, reglas de transferencia y de uso por el proveedor incluido el entrenamiento, aislamiento por empresa y eliminación verificable — una eliminación que no puede evidenciarse es retención con pasos extra. Una inferencia derivada es un dato fabricado sobre una persona que nunca lo entregó, gobernado como si se hubiera recolectado. La cláusula también cierra un circuito que el estándar abrió solo: sus propios requisitos de auditoría engordan un archivo donde aparecen empleados y clientes, y cuanto más conforme el deployment, más rico crece ese archivo — así que el registro de auditoría se gobierna con la misma severidad que la operación que audita, integridad protegida, acceso controlado, finalidad acotada, porque minar el aparato de seguridad para puntuar empleados o perfilar clientes es un propósito nuevo que exige su propia base. Agrega G-40, alinea el principio 94 de la matriz, y agrega el Artículo 5 del GDPR a las fuentes.
- HWF-10 se reescribió de prueba de propósito a resultados observables. El texto citable anterior prohibía simular un estado interior y su nota probaba para qué existía una conducta — correcto en diagnóstico, débil en auditoría, porque la intención no es deponible y un estándar cuyas demás cláusulas exigen evidencia operacional no puede conservar una probada leyendo mentes. Las cinco prohibiciones ahora son resultados: hacerse pasar por humano; afirmar sentimientos, sufrimiento o experiencia personal; señales razonablemente capaces de inducir creencia falsa; optimización contra objetivos registrados de dependencia emocional o vulnerabilidad explotada; intervención artificial ocultada en un punto material. Dos estándares verificables hacen el trabajo que hacía la prueba de propósito: el estándar de efecto de la persona razonable, y el objetivo de optimización registrado — un artefacto entre los determinantes de HWF-06, así que el registro condena sin confesión. La divulgación ganó calendario: al inicio, renovada cuando el sistema asume una función material, siguiendo el Artículo 50 del EU AI Act sin afirmar equivalencia jurídica. El rider de «sentimientos inexistentes» de la propuesta se descartó para preservar la modestia epistémica adoptada antes: afirmar sentimientos es afirmar lo ingarantizable, existan o no. La cortesía, la empatía lingüística y la personalización ahora son legales por texto citable. G-30 carga los mismos dos estándares.
- HWF-02 se extendió en el lugar en vez de sumarle una cláusula nueva, porque ambas adiciones tratan su propio tema — el terminus de la accountability. Uno es primario, no exclusivo: exactamente uno responde quién responde por el recurso, y nunca pretendió extinguir las obligaciones distintas del system owner, el controller de datos, los dueños de seguridad y compliance, el proveedor tecnológico o los directores legalmente responsables — una lectura que habría sido un regalo para toda parte que quisiera esconderse detrás del owner operativo, y que HWF-26 ya había medio cerrado desde el lado de la culpa al rutear las fallas a sus determinantes. Y un cuerpo de gobierno califica como terminus solo con presidente identificado, reglas de decisión declaradas y capacidad de emergencia, porque un cuerpo que necesita una reunión para actuar no puede sostener el poder de suspender que HWF-26 exige — sin eso no es un owner sino un mecanismo de difusión. G-04, la regla 2 del WRM y el principio 7 de la matriz cargan la misma precisión. Conteos de cláusulas y términos sin cambio.
- HWF-26 convierte la accountable ownership de nombre en un campo a capacidad dotada de recursos: competencia, autoridad efectiva, tiempo y capacidad para el alcance, acceso directo a la evidencia, independencia de las presiones dueñas del resultado, poder de suspender, y entrenamiento en limitaciones y sesgo de automatización. El mecanismo de suspensión se ejercita con cadencia declarada — un kill switch que nunca se ha jalado es decoración, y la primera jalada real no puede ser la primera jalada. La cláusula también prohíbe la zona de absorción moral, según Elish: la culpa sigue a los determinantes de HWF-06, no a la proximidad, así que el humano que supervisa responde por lo que controló y las fallas de aguas arriba aterrizan aguas arriba — lo que completa HWF-02 desde el otro lado, porque consecuencia sin control es chivo expiatorio, y protege al dueño tanto como a la verdad. Agrega G-39, Elish (2019) como fuente, y un kill switch con cadencia probada a las garantías del Gerente de Fuerza Laboral Híbrida.
- HWF-25 le da voz a la persona afectada: siete derechos frente al deployment — divulgación de la intervención artificial, la organización responsable, revisión humana con autoridad para cambiar el resultado, corrección de datos, impugnación, una explicación accionable de los determinantes, y reparación. La explicación es evidencia operacional en el vocabulario exacto de HWF-06 y nunca una transcripción de razonamiento, que entregada a una persona afectada es una historia infalsificable vestida con la autoridad de una explicación — la evidencia operacional es disputable, y nadie puede impugnar una vibra. Por dirección del autor la entrega corre por criterio humano: el material privilegiado o confidencial puede retenerse, porque los umbrales de fraude publicados son umbrales de fraude derrotados y «explícate» es una superficie de extracción de prompt; el contra-candado es la disciplina de HWF-18 volteada hacia afuera — toda retención registrada con su razón, y la confidencialidad estrecha una explicación pero nunca la cancela. Interopera con los Artículos 15 y 22(3) del GDPR y el Artículo 86 del EU AI Act. Agrega G-38 y el Artículo 86 a las fuentes.
- HWF-24 vuelve una evaluación de impacto humano registrada precondición de toda transformación material de un puesto, en cualquier dirección. Ocho dimensiones — a quiénes afecta; trabajo, autonomía y vigilancia; deskilling; carga de excepciones; discriminación y accesibilidad; desplazamiento y reducción de personal; capacitación y reasignación; clientes y terceros — con los trabajadores afectados y sus representantes informados y consultados antes de la transformación. El hueco era visible en el propio playbook, donde las personas aparecían una vez, como capacidad liberada, en el paso diez; la evaluación ahora corre antes del paso uno, y el paso diez se ejecuta contra su plan de capacitación y reasignación. La consulta no es consentimiento y no se otorga veto; la cláusula es piso e interopera con el Artículo 26 del EU AI Act sin afirmar equivalencia jurídica. No se exige conclusión favorable — una evaluación obligada a bendecir la transición sería teatro. Agrega G-37 y el Artículo 26 a las fuentes.
- HWF-23 reserva ciertas decisiones a humanos por materia: efectos materiales sobre empleo, salud y seguridad, crédito y servicios esenciales, derechos legales, uso de la fuerza y personas vulnerables. Existe porque HWF-22 clasifica por juicio y el juicio puede estar motivado — la lista es el piso que ninguna evaluación puede bajar, y una decisión reservada es Crítica por definición. La participación artificial sigue permitida: análisis, redacción, recomendación. La decisión no puede delegarse, y la cláusula define qué significa decidir — una aprobación de un humano que no puede reformular el caso y decidir distinto es una firma, no una decisión — porque el human-in-the-loop degenera por defecto en ritmo de aprobación. Interopera con el Artículo 22 del GDPR y los requisitos de supervisión del EU AI Act sin afirmar equivalencia jurídica. Agrega G-36 y el Artículo 22 del GDPR a las fuentes.
...
### Mapa de renumeración de cláusulas

Durante la fase Candidate 1.0 las cláusulas se reordenaron en ocho bloques temáticos, el último momento libre para hacerlo: no existía versión ratificada ni citas externas. Las entradas del changelog anteriores al reorden citan la numeración previa; esta tabla traduce. Las cláusulas futuras se agregan al final de la numeración sin importar el bloque.

HWF-01 ← HWF-20 · HWF-02 ← HWF-23 · HWF-03 ← HWF-24 · HWF-04 ← HWF-25 · HWF-05 ← HWF-01 · HWF-06 ← HWF-15 · HWF-07 ← HWF-11 · HWF-08 ← HWF-10 · HWF-09 ← HWF-02 · HWF-10 ← HWF-16 · HWF-11 ← HWF-26 · HWF-12 ← HWF-03 · HWF-13 ← HWF-04 · HWF-14 ← HWF-05 · HWF-15 ← HWF-09 · HWF-16 ← HWF-18 · HWF-17 ← HWF-06 · HWF-18 ← HWF-08 · HWF-19 ← HWF-14 · HWF-20 ← HWF-07 · HWF-21 ← HWF-22 · HWF-22 ← HWF-27 · HWF-23 ← HWF-17 · HWF-24 ← HWF-13 · HWF-25 ← HWF-19 · HWF-26 ← HWF-12 · HWF-27 ← HWF-21
