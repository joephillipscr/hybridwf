...
| 94 | Data protection | Privacy, access, minimisation and retention principles. | Data governance, consent, retrieval filters, retention and deletion — covering derived inferences and the audit logs themselves (HWF-42). | U |
...
**Acronyms do not translate. WRM, HWFA and the HWF- clause identifiers stay identical in every edition of this standard, present and future; only the words they expand to are localised. A reader who cites WRM or HWF-44 in any language is pointing at the same thing.**
...
- The twenty-seven clauses were reordered into eight thematic blocks and renumbered so that the first digit names the block: 0x Human foundation, 1x The category, 2x Accountability, 3x Authority and operational control, 4x Evidence and data, 5x Risk and validation, 6x Lifecycle, 7x Conformance — in the last free moment such a change will ever have: no ratified version, no seated Board, no external citations. The old numbering was archaeological, recording the order in which review cycles produced the clauses; the new one records the architecture, a citation now carries its own context the way an HTTP status code does, and the supreme clause keeps the number its rank deserves — the foundation is block zero, so HWF-01 still opens the document. Format and citations stay flat HWF-nn. Earlier changelog entries cite the previous numbering, translated by the published renumbering map. The versioning policy fixes the growth rule: a new clause takes the next free number inside its block’s decade, and a block that reaches nine clauses is asking to be split at a major version, not extended.
...
During the Candidate 1.0 phase the clauses were reordered into eight thematic blocks and renumbered so that the first digit names the block — block 0 is the human foundation — in the last free moment such a change will ever have: no ratified version and no external citations existed. Changelog entries older than the reorder cite the pre-reorder numbering; this table translates.
